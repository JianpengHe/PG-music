...
exports.Mysql = exports.EMysqlFieldFlags = exports.EMysqlFieldType = exports.MysqlBuf = exports.SHA1 = exports.Buf = exports.getNumberLen = exports.ReliableSocket = exports.SubReadStream = exports.RecvStream = exports.TypedEventEmitter = exports.recvAll = void 0;
const crypto = require("crypto");
const events = require("events");
const net = require("net");
const stream = require("stream");
const recvAll = (stream) => new Promise((resolve, reject) => {
    const body = [];
...
class RecvStream {
    ...
    newTask() {
        ...
        if (this.task.type === "buffer") {
            ...
            this.tempBufferSize = 0;
            this.task.callback(new SubReadStream(this.sourceStream, this.task.readSize, () => {
...
class ReliableSocket {
    constructor(options, reliableConnectOpts) {
        ...
        this.getSocketSync = () => new Promise((resolve, reject) => {
            try {
...
exports.getNumberLen = getNumberLen;
class Buf {
    ...
    UIntLEToBuffer(number, byteLength) {
        byteLength = byteLength || (0, exports.getNumberLen)(number, true);
    ...
    IntLEToBuffer(number, byteLength) {
        byteLength = byteLength || (0, exports.getNumberLen)(number, false);
        const buf = Buffer.alloc(byteLength);
        if (byteLength === 8) {
    ...
    IntBEToBuffer(number, byteLength) {
        byteLength = byteLength || (0, exports.getNumberLen)(number, false);
        const buf = Buffer.alloc(byteLength);
        if (byteLength === 8) {
    ...
    writeIntBE(number, byteLength, offset) {
...
class MysqlBuf extends Buf {
    ...
    writeStringLenenc(string, offset) {
        return this.writeStringPrefix(string, len => {
...
(function (EMysqlFieldFlags) {
    ...
    EMysqlFieldFlags[EMysqlFieldFlags["auto_increment"] = 512] = "auto_increment";
    EMysqlFieldFlags[EMysqlFieldFlags["timestamp"] = 1024] = "timestamp";
    EMysqlFieldFlags[EMysqlFieldFlags["set"] = 2048] = "set";
})(EMysqlFieldFlags = exports.EMysqlFieldFlags || (exports.EMysqlFieldFlags = {}));
class Mysql extends TypedEventEmitter {
    constructor(connect) {
        ...
        this.sendLongData = (param, statement_id, param_id, sock) => new Promise(resolve => {
            const tempBufs = [];
            let tempBufsLen = 0;
        ...
        this.format = ({ headerInfo, data }) => data.map(row => headerInfo.reduce((obj, header, i) => ({ ...obj, [header.name]: row[i] }), {}));
        this.query = (sql, params) => new Promise((resolve, reject) => {
            this.taskQueue.push({
                sql,
                params,
                callback: (err, value) => {
        ...
        this.reliableSocket = new ReliableSocket({ host: connect.host ?? "127.0.0.1", port: connect.port ?? 3306 }, {
            onConnect: socket => {
                this.socket = socket;
    ...
    async login() {
        ...
        (0, exports.SHA1)(Buffer.concat([info.auth_plugin_data_part_1, info.auth_plugin_data_part_2, (0, exports.SHA1)(password_sha1)])).forEach((byte, i) => {
    ...
    getPrepare(sql) {
        ...
        return new Promise((resolve, reject) => this.reliableSocket.getSocket(async (sock) => {
    ...
    async tryToConsume(times = 0) {
        ...
        if (prepare.paramsNum !== params.length) {
            callback(new Error(`入参与预处理语句的参数对不上。入参数量${params.length}，需要参数${prepare.paramsNum}，预处理语句${sql}`));
            this.task = undefined;
            this.tryToConsume();
            return;
        }
        ...
        this.reliableSocket.getSocket(async (sock) => {
            ...
            for (let index = 0; index < params.length; index++) {
                ...
                if (typeof param === "number") {
                    const len = (0, exports.getNumberLen)(param, false, true);
