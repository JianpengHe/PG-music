...
const save_request_cache = ({ data: { songID, lyric }, item }) => {
  fs.writeFile(PATH + (songID || item?.param?.songID) + ".qqlyric", Buffer.from(lyric, "hex"), () => {});
};
...
const getLyric = async song_ids =>
  await Promise.all(
    (
      await request(
        (song_ids[0].song_id ? song_ids.map(({ song_id }) => song_id) : song_ids).map(GetPlayLyricInfo),
        ob => {
        ...
        new Promise(r => {
          ...
          if (lyric.length) {
            zlib.inflate(LyricDecode(lyric, lyric.length), (err, d) => {
              ...
              const delSongInfos = song_ids[i].singer ? String(song_ids[i].singer).split("、") : [];
